...
import { CONFIG, NEXT_PUBLIC_URL } from "../config";
import Link from "next/link";

const Carousel: FC<{ news: TNewsItem[] }> = ({ news }) => {
  return (
    <div className="container">
        ...
        <div className="carousel__wrapper">
          {news.slice(0, 7).map((item, index) => (
            <div className="item" id="slide-1">
              <img
                src={`${NEXT_PUBLIC_URL}/api/gen-images?id=${index + 1}&news_id=${item.id}`}
                alt=""
                width="418"
                height="418"
              />
      ...
      <article className="instagram">
        <header className="instagram__header">
          <figure>
            <img
              src={`${NEXT_PUBLIC_URL}/images/alphaday-news-logo.png`}
              alt="Jake Dog"
              width="42"
              height="42"
            />
        ...
        <footer className="instagram__buttons">
          ...
          <Link href={CONFIG.ALPHADAY} type="button" className="left">
            Alphaday.com
          </Link>{" "}
          <Link href={CONFIG.ALPHADAY} type="button" className="left">
            Alphaday.com
